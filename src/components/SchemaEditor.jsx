...
export default function SchemaEditor({ schema, setSchema, data = [], setData }) {
  ...
  useEffect(() => {
    ...
    console.log("Schema changed: addedCols=", addedCols, "removedCols=", removedCols);
  ...
  const handleCellChange = (rowIdx, key, value) => {
    const updated = localData.map((r, i) => (i === rowIdx ? { ...r, [key]: value } : r));
  ...
  const displayedSchema = {
    ...
    properties: Object.fromEntries(
      Object.entries(originalSchema?.properties || {}).filter(([k]) => !excludedFields[k])
    ),
  };
    ...
    <div className="schema-editor">
      ...
      <div className="data-preview">
        ...
        {isDataLoaded && localData.length > 0 ? (
          ...
          <p className="no-data-msg">⚠️ No data preview available. Please upload a dataset first.</p>
        )}
      </div>
    </div>
  );
